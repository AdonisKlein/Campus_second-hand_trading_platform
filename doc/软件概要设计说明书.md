...
本文只描述系统级架构、组件职责和组件交互。类、接口、状态机细节见[软件详细设计说明书](./软件详细设计说明书.md)，当前代码落地见[软件实现说明书](./软件实现说明书.md)，正式模块 owner 与写入边界见[服务接口清单与数据表归属方案](./服务接口清单与数据表归属方案.md)。
...
核心数据包括 `users`、`items`、`messages`、`trade_orders`、`email_verification`、`content_reports`、`report_actions`、`chat_conversations`、`chat_messages` 和 Spring Session 表。Flyway V1～V6 从空库建立这些结构，订单保存商品和双方资料快照以避免后续资料变更影响历史记录。表级 owner 和迁移对应关系不在本文重复列举，以《服务接口清单与数据表归属方案》为准。
