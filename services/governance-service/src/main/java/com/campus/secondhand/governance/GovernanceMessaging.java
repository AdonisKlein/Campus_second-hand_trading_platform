...
class GovernanceResultListener {
    ...
    @RabbitListener(queues=GovernanceRabbitConfiguration.RESULT_QUEUE) void receive(Message payload)throws Exception{JsonNode n=mapper.readTree(payload.getBody());String eventId=n.path("eventId").asText();try{governance.applyActionResult(new ContentGovernance.ActionResult(eventId,n.path("correlationId").asText(),n.path("type").asText(),n.path("reportId").asLong(),n.path("reason").isMissingNode()?null:n.path("reason").asText()));}catch(DataIntegrityViolationException duplicate){if(!inbox.existsById(eventId))throw duplicate;}}
}
