...
class GovernanceEventStore {
    ...
    String request(ContentReport report,long adminId){String eventId=UUID.randomUUID().toString();Map<String,Object> event=new LinkedHashMap<>();
        event.put("eventId",eventId);event.put("correlationId",eventId);event.put("version",1);event.put("occurredAt",LocalDateTime.now(clock).toString());event.put("producer","governance-service");event.put("type","GovernanceActionRequested");event.put("reportId",report.getId());event.put("targetType",report.getTargetType().name());event.put("targetId",report.getTargetId());event.put("action",report.getDecisionAction().name());event.put("adminId",adminId);
