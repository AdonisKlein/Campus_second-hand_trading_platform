...
public class MessageController {
    ...
    @PostMapping public ApiResponse<MessageView> send(Authentication auth,@Valid @RequestBody SendRequest request){long actor=actors.require(auth).userId();return ApiResponse.created(view(questions.ask(actor,request.itemId(),null,request.replyToId(),request.content())));}
    ...
    public record SendRequest(@NotNull Long itemId, Long replyToId,@NotBlank @Size(max=500) String content){}
